...
def feature_extractions(directory):
    ...
    Return: A dictionary of inputs for the VGG-16
    """
    features = {}
...
if __name__ == "__main__":
    ...
    with open("features_VGG16.pkl", "wb") as f:
       dump(features, f)
...
def load_features(dataset):
    ...
    """
    with open("features_VGG16.pkl", "rb") as f:
        features = load(f)
    n_features = {photo_id: features[photo_id] for photo_id in dataset}
...
if __name__ == "__main__":
    ...
    word_model = gensim.models.Word2Vec(sentences=sentences, vector_size=100, min_count=2, window=5, epochs=100, sg = 1)
    #save word2vec
...
def define_model(max_length, vocab_size, embedding_size, pretrained_weights):
    ...
    #Load VGG16 model
    vgg_conv = keras.applications.vgg16.VGG16()
    ...
    img_dp1 = keras.layers.Dropout(rate=0.2)(vgg_conv.layers[-2].output)
    ...
    model = keras.models.Model(inputs = [vgg_conv.input, text_inputs], outputs=decoder_outputs)
